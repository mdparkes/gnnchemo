...
Per Liang et al, only genes that participate in an edge are included in the NodeInfoDict. The node set will only include
genes that participate in directed edges.
...
can be inferred without the KEGG database. It was done this way because earlier work used the KEGG database, and rather
than rewriting a bunch of code from scratch, existing code and data structures were repurposed.
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Creates Reactome node information and adjacency dictionaries in the same style as those derived "
                    "from the KEGG BRITE orthology (for compatibility with existing GNN modeling scripts)."
    )
    parser.add_argument(
        "--data_dir",
    ...
    # which pathways were used by Liang et al.
    level_c_dict = {}
    pathways_used = []
